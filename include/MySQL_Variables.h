...
class MySQL_Variables {
	...
	/* arrays of function pointers that verify and update server variables
	 * The typedefs for function pointers are above
	 */
	verify_var verifiers[SQL_NAME_LAST];
	update_var updaters[SQL_NAME_LAST];

public:
	/* This flag determines if session has connection to backend. If there is no connection
	 * to backend then updating backend variables does not make sense
	 *
	 * When session is created, the backend connection is absent. Only client connection exists.
	 * This variable is checked to avoid setting backend connection when it is NULL
	 *
	 * This variable is checked only in one place: session handler method where verification loop
	 * takes place.
	 * */
